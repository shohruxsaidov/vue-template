...
import type { VAvatarProps, VAvatarColor } from '@/components/base/avatar/VAvatar.vue'
import * as listData from '@/data/layouts/flex-list-v1'
      ...
      <div class="flex-list-wrapper flex-list-v1">
        ...
        >
          <template #image>
            <img
              class="light-image"
              src="@/assets/illustrations/placeholders/search-4.svg"
              alt=""
            />
            <img
              class="dark-image"
              src="@/assets/illustrations/placeholders/search-4-dark.svg"
              alt=""
            />
          </template>
        </VPlaceholderPage>
            ...
            <transition-group name="list" tag="div">
              ...
              <div v-for="item in filteredData" :key="item.id" class="flex-table-item">
                ...
                <div class="flex-table-cell" data-th="Status">
                  <span v-if="item.status === 'Online'" class="tag is-success is-rounded">{{
                    item.status
                  }}</span>
                  <span v-if="item.status === 'Working'" class="tag is-info is-rounded">{{
                    item.status
                  }}</span>
                  <span v-if="item.status === 'Suspended'" class="tag is-danger is-rounded">{{
                    item.status
                  }}</span>
